...
function PartyLine() {
	...
	this.ForkLink = require('./procs/forklink.js');
	this.Sap = require('./sap.js');
...
PartyLine.prototype.addWorker = function(new_worker) {
	var new_proc = (new_worker || new this.Worker(partyline));
...
PartyLine.prototype.addLocalProcess = function(task, event_callback) {
	...
	var new_worker = new partyline.Worker(partyline); //create a worker
...
PartyLine.prototype.removeWorkerById = function(worker_id) {
	var self = this;
	var found = [];
	self.process_bus.forEach(function(proc, index, ar){
	...
	found.forEach(function(pos, index, ar){
		self.process_bus.removeWorkerFromStack(pos);
	});
};
...
PartyLine.prototype.emit = function(packet) {
	...
	if (routed_packet.context == "bus") { //asking the bus controller
		switch(routed_packet.type) {
			case "setListenContext" : 
				self.Sap.registerService(self.id, packet.from, packet.listen_context);
				break;
			case "sapQuery":
				self.Sap.queryService(routed_packet.service_context, function(services){
					routed_packet = {
						context: packet.from,
						from: "bus",
						type: "serviceList",
	...
	this.process_bus.forEach(function(proc, index, ar){
		if (proc.matchContext(routed_packet.context)) {
			proc.send(routed_packet);
		}
	});
};
